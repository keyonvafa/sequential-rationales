...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    word_dropout_mixture: float = field(
        default=0.0,
        metadata={
            "help": "Proportion of the time to use word dropout objective "
                    "instead of full sequence objective. Note that this is "
                    "different from the probability of dropping out a token."
        },
    )
    word_dropout_type: Optional[str] = field(
        default=None,
        metadata={
            "help": "Type of word dropout to perform (only if using word "
                    "dropout. Use 'uniform_length' for first sampling a "
                    "sequence length uniformly at random and then considering "
                    "all sequences uniformly at random given the length. Use "
                    "'inverse_length' for dropping each token with "
                    "probability (1 - 1/seq_len)."
        },
    )
...
@register_model_architecture("transformer_lm", "transformer_lm_majority_class")
def transformer_lm_majority_class(args):
